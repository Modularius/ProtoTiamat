...
pub async fn get_user_id_from_session_id(
    ...
    let session = server.get_session(&session_id).ok_or_else(|| {
        ServerFnErrorErr::ServerError(format!(
...
#[component]
pub fn UserGuard<C>(children: TypedChildrenFn<C>) -> impl IntoView
where
    C: IntoView + 'static,
{
    let top_level_context = use_context::<TopLevelContext>().expect_context();

    let session_id = top_level_context.session_id_expect();
